...
class Neuron:
    ...
    def LOG_DATA(self, a_result, b_Bias):
        file = open('data/data.txt', 'a')
        file.write(f' LOG_ID {str(datetime.now()).replace(' ', '-')} X {self.x} W {self.w} B {b_Bias} A {a_result} ANSWER {self.answer} NRETURN {self.nreturn} \n')
        file.close()
